...
StatsValue& StatsValue::operator=(const LiveValue& v) {
    std::visit(overload {
                   ...
                   [&](const std::vector<SunSpecBlock<double>>& v) {
                       auto pOut = std::get_if<std::vector<SunSpecBlock<MeasuredValue<double>>>>(&m_variant);
                       if (!pOut) {
                           m_variant = std::vector<SunSpecBlock<MeasuredValue<double>>>(v.size());
                       }
                       auto& out = std::get<std::vector<SunSpecBlock<MeasuredValue<double>>>>(m_variant);
                       out.resize(v.size());
